...
class UNet(nn.Module):
    ...
    def __init__(self, in_channels, out_channels_seg, out_channels_reg, dim=2, depth=5, initial_channels=32, normalization=True):
        ...
        for i in range(self.depth):
            ...
            self.down_path_seg.append(ConvBlock(prev_channels, current_channels, dim, normalization))
            self.down_path_reg.append(ConvBlock(prev_channels, current_channels, dim, normalization))
    ...
    def forward(self, x):
        ...
        for i, (down_seg, down_reg) in enumerate(zip(self.down_path_seg, self.down_path_reg)):
            ...
            if i < self.depth - 1:
                ...
                x_seg = F.interpolate(x_seg, scale_factor=0.5, mode='bilinear' if self.dim == 2 else 'trilinear',
                                  align_corners=True, recompute_scale_factor=False)
                x_reg = F.interpolate(x_reg, scale_factor=0.5, mode='bilinear' if self.dim == 2 else 'trilinear',
                                      align_corners=True, recompute_scale_factor=False)
