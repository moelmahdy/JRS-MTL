...
from . import sedd_unet


class SEDDNet_single(nn.Module):
    '''
    ...
    def __init__(self, dim, n, classes= 6, scale=1, depth=5, initial_channels=64, normalization='instaNorm'):
        ...
        self.unet = sedd_unet.UNet(in_channels=n, out_channels_reg=dim * n, out_channels_seg= classes * n, dim=dim,
                                 depth=depth, initial_channels=initial_channels, normalization=normalization)
    ...
    def forward(self, input_image):
        ...
        scaled_logits, scaled_disp_t2i  = self.unet(scaled_image)
        scaled_disp_t2i = scaled_disp_t2i.squeeze(0).reshape(self.n, self.dim, *scaled_image_shape)  # (n, 2, h, w) or (n, 3, d, h, w)
        scaled_logits = scaled_logits.squeeze(0).reshape(self.n,  self.num_Classes, *scaled_image_shape)  # (n, classes, h, w) or (n, classes, d, h, w)
        ...
        if self.scale < 1:
            ...
            logits = torch.nn.functional.interpolate(scaled_logits, size=original_image_shape,
                                                     mode='bilinear' if self.dim == 2 else 'trilinear',
                                                     align_corners=True)
        else:
        ...
        probs = F.softmax(logits, dim=1)# (n, classes, h, w) or (n, classes, d, h, w)
        _, predicted_label = torch.max(probs, dim=1, keepdim=True)# (n, 1, h, w) or (n, 1, d, h, w)
        ...
        res.update({'disp_t2i': disp_t2i, 'scaled_disp_t2i': scaled_disp_t2i, 'warped_input_image': warped_input_image,
               'template': template})
        ...
        res.update({'scaled_template': scaled_template})
